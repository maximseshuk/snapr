...
interface GetBackupsColumnsParams {
  t: TFunction
  canDownload: boolean
  onDownload: (backupId: string) => void
  onDownloadPart: (partFilename: string) => void
...
const getBackupsColumns = ({
  t,
  canDownload,
  onDownload,
  onDownloadPart,
}: GetBackupsColumnsParams): ColumnDef<Backup>[] => [
  {
...
export const JobBackupsTab = ({ backups, onDownload, onDownloadPart, canDownload = true }: JobBackupsTabProps) => {
  ...
  const backupsColumns = useMemo(
    () => getBackupsColumns({ t, canDownload, onDownload, onDownloadPart }),
